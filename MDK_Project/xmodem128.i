...
uint8_t	XMODE_SendCmdRecPick(uint8_t	Cmd)
{
		 ...
		 for(Temp=0;Temp<sizeof(XMODEM128);Temp++)	  			
		 {
    	 ...
    	 ((LPC_USART_Type *) (0x40008000))->THR =	Cmd;								
		 if(Cmd == 'C')		delaySysTick(10000);
		 else				
		 {
		 	  do
			 {
...
uint8_t	XMODE_ReceiveData(uint8_t Order)
{
	...
	do
	{
		 	...
		 	if(( MyData->PacketNumber1 == PacketNumA)&&(MyData->PacketNumber0 == PacketNumB)&&(MyData ->RecEn == 0xAA))
			{
				...
				if( XOMDE_CRC16((uint8_t *)(UARTBuffer+3), 130) == 0x00)
				{
					for(Step=3;Step<131;Step++)
					{
						...
						if( FlashAddr > 0XFFFFF)
						{
							XMODE_SendCmdRecPick( 0x10 );					
							Retry=0x00;										
						}		 
					}
					PacketNumA+=1;
		...
		if( MyData->StartOfHeader == 0x04)		  				
		{
			   EndCnt	++;
			   CMD		=0x15;	   
		}
		if( MyData->StartOfHeader == 0x18)		  				
		{
			   CancelCnt ++;
			   CMD		=0x15;	   
		}
												
	}while((EndCnt != 5)&&(CancelCnt != 5)&& Retry );	
	XMODE_SendCmdRecPick( 0x06 );
